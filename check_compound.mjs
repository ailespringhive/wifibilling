const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const key = 'standard_';

fetch(endpoint + '/databases/' + databaseId + '/collections/billings/documents?queries[]=' + encodeURIComponent('equal("customerId", ["CUST-243964"])') + '&queries[]=' + encodeURIComponent('orderDesc("$createdAt")'), {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   console.log(d);
});
