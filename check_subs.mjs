const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const key = 'standard_';

fetch(endpoint + '/databases/' + databaseId + '/collections/subscriptions/documents', {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   console.log(JSON.stringify(d.documents.map(x => ({id: x.$id, customerId: x.customerId, status: x.status, collectorId: x.collectorId})), null, 2));
});
