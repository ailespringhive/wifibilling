const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const key = 'standard_';

fetch(endpoint + '/databases/' + databaseId + '/collections/users_profile/documents/CUST-243964', {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   console.log('Got profile by ID:', d.$id);
}).catch(e => console.log('Not found by ID'));

fetch(endpoint + '/databases/' + databaseId + '/collections/users_profile/documents?queries[]=' + encodeURIComponent('equal("userId", "CUST-243964")'), {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   console.log('Got profile by userId:', d.documents ? d.documents.length : 'error');
});
