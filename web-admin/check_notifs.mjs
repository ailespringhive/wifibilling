const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const key = 'standard_';

fetch(endpoint + '/databases/' + databaseId + '/collections/mobile_notifications/documents', {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   console.log("Total notifications:", d.total);
   if (d.documents && d.documents.length > 0) {
     console.log("Latest 2 notifications:", JSON.stringify(d.documents.slice(0, 2), null, 2));
   }
});
