const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const key = 'standard_';

fetch(endpoint + '/databases/' + databaseId + '/collections/mobile_notifications/documents?queries[]=orderDesc("$createdAt")&queries[]=limit(5)', {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   if (d.documents && d.documents.length > 0) {
     console.log("Latest notifications:", JSON.stringify(d.documents, null, 2));
   } else {
     console.log(JSON.stringify(d));
   }
});
