const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const collectionId = 'mobile_notifications';
const key = 'standard_';

async function testSubmit() {
    console.log("Testing POST to mobile_notifications...");
    const res = await fetch(`${endpoint}/databases/${databaseId}/collections/${collectionId}/documents`, {
        method: 'POST',
        headers: {
            'X-Appwrite-Project': projectId,
            'X-Appwrite-Key': key,
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            documentId: 'unique()',
            data: {
              technicianId: 'some-tech-id',
              collectorId: 'collector-app', 
              title: 'New Repair Ticket',
              message: 'xample reported an issue',
              type: 'repair',
              isRead: false,
              createdAt: new Date().toISOString()
            },
            permissions: [
              'read("user:some-tech-id")',
              'update("user:some-tech-id")',
              'delete("user:some-tech-id")'
            ]
        })
    });
    const data = await res.json();
    console.log("Result:", JSON.stringify(data, null, 2));
}

testSubmit().catch(console.error);
