const endpoint = 'https://appwrite.springhive.co/v1';
const projectId = '69c4840a0022d6824d83';
const databaseId = 'wifi_billing_db';
const key = 'standard_';

fetch(endpoint + '/databases/' + databaseId + '/collections/users_profile/documents', {
  headers: {
    'X-Appwrite-Project': projectId,
    'X-Appwrite-Key': key
  }
}).then(r => r.json()).then(d => {
   const docs = d.documents || [];
   docs.forEach(doc => {
      if(JSON.stringify(doc).includes('CUST')) {
         console.log(doc.$id, doc.userId, doc.firstName, doc.lastName);
      }
   });
});
